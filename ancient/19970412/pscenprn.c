...
void ps_show_world (World *world, int player, FILE *fptr)
{
  ...
  int i, max_x, min_x, max_y, min_y, domain;
  ...
  for (i = 0; i < hexlist->cnt; i++)
    {
      if (hexlist->list[i].x < min_x) min_x = hexlist->list[i].x;
      if (hexlist->list[i].x > max_x) max_x = hexlist->list[i].x;
      if (hexlist->list[i].y < min_y) min_y = hexlist->list[i].y;
      if (hexlist->list[i].y > max_y) max_y = hexlist->list[i].y;
    }
