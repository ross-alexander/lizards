...
int main (int argc, char *argv [])
{
  ...
  div_size = sqrt (players_in_game);

  div_x = div_size;

  div_y = div_size;

  while (div_x * div_y < players_in_game)
    if (random (2) == 0)
      div_x ++;
    else
      div_y ++;

  grid_x = ceil ((float) world_x / div_x);
  grid_y = ceil ((float) world_y / div_y);
  ...
  for (work = 1; work < players_in_game + 1; work ++)
  {
	...
	if (breaker2 == 100)
        {
          printf ("FATAL ERROR: No free locations for new player. Press SPACE.\n");
	...
	for (work3 = grid_y * y; work3 < grid_y * (y + 1); work3 ++)
	{
      ...
      if (breaker == 100)
      {
        printf ("FATAL ERROR: No free dens in location for new player. Press SPACE.\n");
    ...
    do
    {
	 ...
	 breaker3 ++;
      } while ((den_xy_list [work2]->x == 0 || den_xy_list [work2]->x == world_x - 1 ||
	       den_xy_list [work2]->y == 0 || den_xy_list [work2]->y == world_y) &&
	       breaker3 != 100);
	...
	exit (EXIT_FAILURE);
      ...
      get_hex (den_xy_list [work2]->x, den_xy_list [work2]->y, &world_hex);
	...
	default:
      ...
      if ((work3 = terrain_adjacent (den_xy_list [work2]->x, den_xy_list [work2]->y,
	  FERTILE)) > 1)
      {
	breaker = 0;
	...
	{
	  move_coords (work4, den_xy_list [work2]->x, den_xy_list [work2]->y,
		       &x2, &y2);
	...
	world_hex.lizard_pop = 50;
      ...
      if (breaker2 == 100)
      {
        printf ("FATAL ERROR: Home den adjustment failed. Press SPACE.\n");
    ...
    put_hex (den_xy_list [work2]->x, den_xy_list [work2]->y, &world_hex);
