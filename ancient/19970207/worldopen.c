...
int ProcessHex(hex_t world_hex)
{
  ...
  hex_change = 0;
  xy_to_string (world_hex.world_x, world_hex.world_y, work_string);
  ...
  if (world_hex.terrain == CURSED && turn_number > 0)
	...
	if (spy_list [work3]->x_hex == world_hex.world_x &&
	    spy_list [work3]->y_hex == world_hex.world_y)
	  {
  ...
  if (world_hex.terrain == DEN || world_hex.terrain == HOME_DEN)
    {
      work3 = terrain_adjacent (world_hex.world_x, world_hex.world_y, FERTILE);
