...
int main (int argc, char *argv [])
{
  ...
  LzPaths lzpaths(argc, argv);
  if (!lzpaths.gameCode.length() && !lzpaths.dataPath.length())
    {
  ...
  for (loop = 1; loop < players_in_game + 1; loop ++)
  {
    ...
    no_move_returned_flags [loop] = TRUE;
  }
  ...
  while (fgets (work_string, 140, in_fptr) != NULL)
  {
	...
	if (work >= 1 && work <= players_in_game)
	{
	  ...
	  no_move_returned_flags [present_player] = FALSE;
	}
	...
	while (fgets (temp_string2, 140, in_fptr) != NULL)
	{
	...
	if ((temp_fptr = fopen (filename, "wt")) == NULL)
	{
  ...
  if ((bin_fptr = fopen (filename, "wb")) == NULL)
  {
    printf ("FATAL ERROR: Unable to write to %s file. Press SPACE.\n", filename);
...
void add_order (order_t *order)
{
  ...
  if ((order_list [number_of_orders] = malloc (sizeof (order_t))) == NULL)
  {
