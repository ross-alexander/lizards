...
int main (int argc, char **argv)
{
  ...
  sprintf (string, "%sDPL%02d%03d.%s", data_path, disk_player, turn_number,
    game_code);
  ...
  do
  {
    ...
    cprintf ("  Viewing Turn Report for Player %02d, Turn %03d in Game %s.\n\r\n\r",
      disk_player, turn_number, game_code);
...
void view_file (FILE *fptr, char *end_marker)
{
  ...
  while (fgets (string, 255, fptr) != NULL)
  {
    ...
    if (lines_so_far > 21)
    {
      ...
      cprintf ("-- MORE -- Press SPACE to continue, 'M' for menu.");
  ...
  {
    textattr (BLACK + (LIGHTGRAY << 4));
    cprintf ("END OF TURN REPORT. Press SPACE for menu.");
    textattr (LIGHTGRAY + (BLACK << 4));

    getch ();
  } else {
    textattr (BLACK + (LIGHTGRAY << 4));
    cprintf ("END OF SECTION. Press SPACE for menu.");
    textattr (LIGHTGRAY + (BLACK << 4));
