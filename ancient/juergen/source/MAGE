...
class LizCanvas extends Panel
{
     ...
     for (hexY=0; hexY < worldHeight; hexY++)
      {
        ...
        if ((worldHexes[hexBase] & 0x80) == 0) // not visible
          continue;
        if ((worldHexes[hexBase] & 31) == HOMEDENHEX)
        {
               g.setColor(Color.lightGray);
               g.drawRect(xp+5,yp+6,12,7);
               g.setColor(Color.black);
               if (flagUp) // up down up
               {
                 g.fillRect(xp+5,yp+6,4,5);
                 g.fillRect(xp+9,yp+7,4,5);
                 g.fillRect(xp+13,yp+6,4,5);
               }
              else  // down up down
              {
                 g.fillRect(xp+5,yp+7,4,5);
                 g.fillRect(xp+5,yp+6,4,5);
                 g.fillRect(xp+5,yp+7,4,5);
              }
              flagUp = !flagUp;
  ...
  public void addHexLizards(int hexX, int hexY, int nbrId, int lizId)
  {
    ...
    worldHexes[hexBase+1+lizId] += (nbrId+1)*10;
  }
  ...
  public void selectHex(int hexX, int hexY, int newIcon)
  {
    ...
    {
       worldHexes[hexBase] &= ~31;
       worldHexes[hexBase] |= newIcon;
    }
...
*/
  ...
  void paintHex(int hexX, int hexY, Graphics g)
  {
         ...
         int slices[] = new int[6];
         String s;
         Color colors[] = new Color[5], textcolor;
...
//         s = hexX+","+hexY;
         ...
         g.drawString(""+sum,xp-w+6,yp-6);
}
  }

  public void paintClipped(int xc, int yc, int wc, int hc,Graphics g)
   {
...
*/
  ...
  int translateButton(int btn)
  {
   ...
   public void mouseClick(int x, int y, int btn, int nbrId, int lizId)
   {
     Point point;
     int oldSelX,oldselY;
     Graphics g;
     ...
     if (btn == 9)
      {
         addHexLizards(point.x,point.y,nbrId,lizId);
      }
     else
      if (btn==19)
