...
int main (int argc, char *argv [])
{
  ...
  for (work2 = 1; work2 < players_in_game + 1; work2 ++)
	fprintf (fptr, " %d - Player %02d, '%s' (%3s) Clan.\n",
		 player[work2].home_den, work2, player[work2].clan_name,
                 player[work2].code);
  ...
  if (victory_points_switch)
    for (work = 1; work < players_in_game + 1; work ++)
      if (player[work].home_den > 0)
	scores [work] += ((int) player[work].home_den *
	  home_owned_worth);
  ...
  for (work = 0; work < players_in_game + 1; work ++)
  {
	...
	fprintf (fptr, " %02d - Player %02d - EXTINCT - '%s' (%3s) Clan.\n",
		 work2, scorers [work2], player[scorers [work2]].clan_name,
		 player[scorers [work2]].code);
      else
	fprintf (fptr, " %02d - %6d Player %02d, '%s' (%3s) Clan.\n",
		 work2, scores [work2], scorers [work2],
		 player[scorers [work2]].clan_name, player[scorers [work2]].code);
  ...
  if (victory_points_switch)
  {
	...
	if (scorers [work2] == work)
      ...
      if (player[work].home_den >= 1 && victory_points_for_victory > 0 &&
        ...
        if (work3 == winner_player)
	{
	  fprintf (fptr, "\n * '%s' Clan YOU HAVE WON THIS GAME OF LIZARDS!\n",
		     player[winner_player].clan_name);
    ...
    for (work = 1; work < players_in_game + 1; work ++)
      if (player[work].home_den >= home_dens_for_victory &&
	  home_dens_for_victory > 0)
      {
	for (work3 = 0; work3 < players_in_game + 1; work3 ++)
	{
