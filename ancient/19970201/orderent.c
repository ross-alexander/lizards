...
int main (int argc, char *argv[])
{
  ...
  for (player = 1; player < players_in_game + 1; player ++)
  {
    ...
    set_field (order_template, player_orders [player], 6, player_clan_name [player]);
    set_field (order_template, player_orders [player], 7, player_code [player]);
  ...
  sprintf (string, "%s%s%03d.%s", data_path, TEXT_ORDS_F, turn_number,
    game_code);
