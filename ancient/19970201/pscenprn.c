...
void centre_points (anchor_t *base, int centre_x, int centre_y)
{
  ...
  for ( loop = 0; loop < base->num_elements; loop ++ )
  {
    ...
    x = ptr->x - centre_x + (world_x / 2);

    if (ptr->x % 2 != x % 2)
      x += 1;

    if (x < 0) x += world_x;
    if (x >= world_x) x = x % world_x;

    y = ptr->y - centre_y + (world_y / 2);

    if (y < 0) y += world_y;
    if (y >= world_y) y = y % world_y;
