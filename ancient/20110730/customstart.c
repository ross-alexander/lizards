...
void CustomStartup(World *world, char *startfilename)
{
  ...
  sprintf (filename, "%s%s.%s", data_path, game_code, CUSTSTAR_LOG_FILE);
...
void build_startup (int p, FILE *template_fptr, FILE *log_fptr)
{
  ...
  empty_hex.bridge_here = LZ_FALSE;
  ...
  if (hex_list [home_hex].terrain == DEN && fertiles > 0)
  {
    ...
    lay_template (p, tmpl, hex_list, log_fptr);

    world->player[p].home_den = 1;
    free (hex_list);
    return;
  }
  else
	...
	exit (EXIT_FAILURE);
      ...
      build_startup (p, default_fptr, log_fptr);
...
void lay_template (unsigned int p, unsigned int tmpl, hex_t *hexes,
                   FILE *log_fptr)
{
  char string [256];
