...
int check_order_format (unsigned int *, field_t **, char **data);
...
int main (int argc, char *argv[])
{
  ...
  other_bit_template = load_template ("OTHFORM.TEM",
				      &other_bit_template_fields);
  ...
  for (player = 1; player <= world->num_players; player ++)
					    ...
					    sizeof (char *))) == '\0')
	{
      ...
      set_field (order_template, player_orders [player], 1, itoa (world->turn, string, 10));
  ...
  sprintf (string, "%s%s.%s%03d", data_path, game_code,
	   TEXT_ORDS_F, world->turn);
  ...
  while (player != 0)
    {
      for (loop = player; loop <= world->num_players &&
      ...
      if (loop != world->num_players + 1)
	{
      ...
      */
      switch (enter_template (order_template,
			      player_orders [player], start_field,
			      &field_in, check_order_format, order_ditto,
			      order_skip_to_field))
	  {
	  case ERASE_KEY:
	    clear_error ();
	  ...
	  case DOWN_FORM_KEY:
		...
		start_field = ORDER_FORM_START_FIELD;
	  default:
	    for (loop = player + 1; loop <= world->num_players &&
		   strcmp (world->player[loop].code, "****") == 0; loop ++);
...
int check_order_format (unsigned int *field, field_t **template, char **data)
{
  ...
  if (data [*field] != '\0')
  {
	...
	for (loop = 0; loop < world->num_spies; loop ++)
	  if (world->spylist[loop]->x_hex == x &&
	      world->spylist [loop]->y_hex == y &&
	      world->spylist [loop]->player == player)
	    break;

	if (loop == world->num_spies)
	{
