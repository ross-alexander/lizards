...
void chant_order (order_t *order)
{
  ...
  move_coords(world, order->moves [0], order->x_hex, order->y_hex, &x, &y);
  move_coords(world, order->moves [1], x, y, &x2, &y2);
  ...
  for (work = 0; work < world->num_orders; work ++)
	...
	world->order_list [work]->order_type == CHANT)
    {
	...
	if (world->order_list [work]->moves [1] != 0)
	  fprintf (fptr, "%s %s) ", dir [world->order_list [work]->moves [0]], dir [world->order_list [work]->moves [1]]);
	else
...
void give_order (order_t *order)
{
  ...
  spy = player_spy_here (world, order->x_hex, order->y_hex, order->player);
  ...
  if (strcmp (world->player[order->moves [0]].code, "****") == 0)
  {
    open_player_mesg (order->player, &fptr);
  ...
  if (world_hex.hex_owner != order->player && spy > -1)
  {
    for (work = 0; work < world->num_spies; work ++)
      if ((world->spylist [work]->x_hex == order->x_hex) &&
	  (world->spylist [work]->y_hex == order->y_hex) &&
	  (world->spylist [work]->player == order->player))
      {
	open_player_mesg (order->player, &fptr);

	fprintf (fptr, " - You give Spy Lizard at %s to Clan '%s'.\n",
		 work_string, world->player[order->moves [0]].clan_name);
	...
	fprintf (fptr, " - Clan '%s' gives you Spy Lizard at %s.\n",
	       world->player[order->player].clan_name, work_string);

	/* fclose(fptr); */

	world->spylist [work]->player = order->moves [0];
      }
  } else {
    open_player_mesg (order->player, &fptr);
  ...
  for (work = 0; work < world->num_orders; work ++)
    if (world->order_list [work] != order &&
	world->order_list [work]->player == order->player &&
	world->order_list [work]->order_type == GIVE)
    {
...
void free_order (order_t *order)
{
  ...
  if (freed > 0)
  {
	...
	fprintf (fptr, " Lizards join the Den population here.\n");
      ...
      if (world_hex.lizard_pop >= MINIMUM_POP_LEVEL && world_hex.den_lizard_type == 0)
	{
	  if (free_colour != 0)
	    {
...
void lob_order (order_t *order)
{
  ...
  switch (new_hex.terrain)
  {
	...
	fprintf (fptr, "\n - Lizards at %s showered with sticks and stones from '%s' Clan Lizards at %s.\n"
		 ...
		 work_string2, world->player[order->player].clan_name, work_string);
  ...
  if (work2 == 0)
  {
    ...
    put_hex(world, x2, y2, &new_hex);

    /* adjust any orders for this hex, downward to compensate for the loss
       of lizards */

    for (work = 0; work < world->num_orders; work ++)
      if (world->order_list [work]->x_hex == x2 &&
          world->order_list [work]->y_hex == y2 &&
	  world->order_list [work]->player == new_hex.hex_owner)
    {
      if (world->order_list [work]->order_type == CHANT)
      {
	if (world->order_list [work]->black_lizards > new_hex.black_lizards)
	  world->order_list [work]->black_lizards = new_hex.black_lizards;
      } else {
	/* adjust order lizards downward to compensate for loss in hex */

	if (world->order_list [work]->red_lizards > new_hex.red_lizards)
	  world->order_list [work]->red_lizards = new_hex.red_lizards;

	if (world->order_list [work]->green_lizards > new_hex.green_lizards)
	  world->order_list [work]->green_lizards = new_hex.green_lizards;

	if (world->order_list [work]->grey_lizards > new_hex.grey_lizards)
	  world->order_list [work]->grey_lizards = new_hex.grey_lizards;

	if (world->order_list [work]->black_lizards > new_hex.black_lizards)
	  world->order_list [work]->black_lizards = new_hex.black_lizards;

	if (world->order_list [work]->yellow_lizards > new_hex.yellow_lizards)
	  world->order_list [work]->yellow_lizards = new_hex.yellow_lizards;
      }
    }
    ...
    fprintf (fptr, "\n - Lizards at %s showered with sticks and stones from '%s' Clan Lizards at %s. %d Lizards are killed!\n",
    	     work_string2, world->player[order->player].clan_name, work_string, work2);
    ...
    if (lizards_in_hex (&new_hex) == 0)
    {
      ...
      hex_attacked_or_moved_into (world, x2, y2, &new_hex);
    }
  }
  ...
  for (work = 0; work < world->num_orders; work ++)
	...
	world->order_list [work]->y_hex == order->y_hex &&
	world->order_list [work]->player == order->player &&
	world->order_list [work]->order_type == LOB)
    {
...
void kill_player_order (order_t *order)
{
  ...
  strcpy (world->player[order->player].code, "****");
