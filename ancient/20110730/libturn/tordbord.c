...
--
---------------------------------------------------------------------- */
void TextOrderConvert(World*);
void TextOrderConvert(World *world)
{
  char filename [255];
  int loop;
  ...
  sprintf (filename, "%s%s.%s%03d", data_path, game_code, TEXT_ORDS_F,
	   world->turn);
...
void add_order (order_t *order)
{
  ...
  if ((world->order_list [world->num_orders] = malloc (sizeof (order_t))) == NULL)
  {
    printf ("FATAL ERROR: Out of memory.\n");
    exit (EXIT_FAILURE);
  }

  *(world->order_list [world->num_orders]) = *order;

  world->num_orders ++;

  if (world->num_orders >= MAX_ORDERS_PER_TURN)
  {
