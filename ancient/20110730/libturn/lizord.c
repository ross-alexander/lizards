...
void hex_attacked_or_moved_into (World *world, int x, int y, hex_t *hex)
{
  FILE *fptr;
  ...
  switch (order->order_type)
  {
    case MOVE:
      printf ("Move Order (P%d-%s %s %s %s).\n",
	      order->player, world->player[order->player].code, work_string,
    ...
    case KILL_PLAYER:
      printf ("Kill Player Order (P%d-%s).\n",
	      order->player, world->player[order->player].code);
