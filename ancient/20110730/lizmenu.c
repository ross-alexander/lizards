...
void change_player (void)
{
  ...
  if (players_in_game == 0)
  {
    cprintf ("\n\r\n\r     LIZARDS! Game %s HAS NO PLAYERS!\n\r",
      game_code);
    return;
  }
  ...
  if (p > MAX_PLAYERS + 1)
  {
    ...
    cprintf ("INVALID player number entered.");
    return;
  }
  ...
  if (strcmp (player[p].code, "****") == 0)
  {
    ...
    cprintf ("NON-EXISTANT or REMOVED player number entered.");
  ...
  set_field (player_template, player_data, 1, itoa (p, string, 10));
  set_field (player_template, player_data, 2, player[p].clan_name);
  set_field (player_template, player_data, 3, player[p].code);
  set_field (player_template, player_data, 3, string);
  ...
  {
    if (player_data [2] != '\0')
      strcpy (player[p].clan_name, player_data [2]);
    else
      player[p].clan_name[0] = '\0';
    ...
    cprintf ("Player number %02d CHANGED in Game %s.",
      player, game_code);
    free_data (player_template, player_data);
    ...
    cprintf ("Player number %02d WAS NOT CHANGED in Game %s.",
      player, game_code);
...
*/
  ...
  cprintf ("\n\r     End of Games.to return to menu.");
...
void delete_game (void)
{
  ...
  if (option == 'Y')
  {
    cprintf ("\n\r\n\r    to DELETE GAME %s or ANY OTHER KEY to abort.",
      game_code);
...
void get_game_player_list (void)
{
  ...
  while (p < MAX_PLAYERS + 1)
  {
	...
	if (player_data [3] != '\0')
          strcpy (player[p].code, player_data [3]);
	else
          player[p].code[0] = '\0';
...
void wipe_player_list (void)
{
  ...
  for (loop = 0; loop < MAX_PLAYERS + 1; loop ++)
  {
    player[loop].code[0] = '\0';
    player[loop].clan_name[0] = '\0';
...
void show_player_list (void)
{
  ...
  for (loop = 1; loop < MAX_PLAYERS + 1; loop ++)
    if (strcmp (player[loop].code, "****") != 0)
    {
      cprintf ("%02d-%-30s(%-3s)\n",
        loop, player[loop].clan_name, player[loop].code);
    }
}
