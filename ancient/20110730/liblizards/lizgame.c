...
void put_switch_list(World *world)
{
  ...
  filename = (char*)alloca(strlen(data_path) + strlen(game_code) +
...
World* get_world (void)
{
  ...
  world->player = (Player*)calloc((MAX_PLAYERS + 1),sizeof(Player));
...
void OpenPlayerFiles(World *world)
{
  ...
  j = strlen(data_path) + strlen(game_code) + strlen("LP_") + 10;
  fname = (char*)alloca(j);

  for (i = 0; i <= world->num_players; i++)
    {
      sprintf(fname, "%s%s.LP%02d_%03d", data_path, game_code,
	      i, world->turn);
