		   ...
		   int green, int grey, int black, int yellow)
{
  ...
  get_hex(world, old_x, old_y, &old_hex);
  get_hex(world, new_x, new_y, &new_hex);
  ...
  put_hex(world, old_x, old_y, &old_hex);
  put_hex(world, new_x, new_y, &new_hex);
}

int combat (World *world,
	    ...
	    int defend_y, int flee_flag)
{
  ...
  *new_yellow = yellow;
  ...
  get_hex(world, attack_x, attack_y, &attack_hex);
  get_hex(world, defend_x, defend_y, &defend_hex);
...
/*	  fclose (fptr1); */
	  ...
	  put_hex(world, attack_x, attack_y, &attack_hex);
	  put_hex(world, defend_x, defend_y, &defend_hex);
...
void capture_lizards (World *world, hex_t *hex, int no_captured,
...
void combat_sea_monster (World *world,
			 int red, int green, int grey, int black, int yellow,
			 ...
			 int attack_y, int *sea_monster)
{
  ...
  *new_yellow = yellow;
  ...
  get_hex(world, attack_x, attack_y, &world_hex);
...
int terrain_adjacent (World *world, int x, int y, int terrain)
{
  int work,
      work2,
      work_x,
  ...
  {
    move_coords (world, work, x, y, &work_x, &work_y);

    if (x != work_x || y != work_y)
    {
      get_hex(world, work_x, work_y, &hex);
...
void in_sight (World *world, int x, int y, int *players)
{
  /* In y,x order! */
  ...
  if ((x % 2) == 1)
  {
    players_here (world, x, y, players);

    for (work = 0; work < 6; work ++)
      players_here (world, (x + one_hex_odd [work][1]),
    ...
    {
      get_hex(world, (x + three_hex_odd [work][1]), (y + three_hex_odd [work][0]), &hex);
    ...
    for (work = 0; work < 18; work ++)
    {
      get_hex(world, (x + three_hex_even [work][1]), (y + three_hex_even [work][0]), &hex);
...
void players_here (World *world, int x, int y, int *players)
{
  int work;
  ...
  for (work = 0; work < world->num_spies; work ++)
	...
	world->spylist [work]->player > 0)
      players [world->spylist [work]->player] = 1;
}
...
--
---------------------------------------------------------------------- */
void get_hex(World *world, int x, int y, hex_t *hex)
{
  memcpy(hex, &world->hexes[x * world->y + y], sizeof(hex_t));
}

void put_hex(World *world, int x, int y, hex_t *hex)
{
  memcpy(&world->hexes[x * world->y + y], hex, sizeof(hex_t));
}

void whirlpool_kill (World *world, int x, int y)
{
  FILE *fptr;
...
void bridge_collapse_kill (World *world, int x, int y)
{
  FILE *fptr;
...
int player_spy_here (World *world, int x, int y, int player)
{
  int work;
...
int moveable_player_spy_here (World *world, int x, int y, int player)
{
  int work,
      spys [20],
      num_spys;

  for (work = 0, num_spys = 0; work < world->num_spies; work ++)
    if (world->spylist [work]->x_hex == x && world->spylist [work]->y_hex == y &&
	world->spylist [work]->player == player)
    {
      spys [num_spys] = work;
  ...
  for (work = 0; work < num_spys; work ++)
    if (world->spylist [spys [work]]->mired == 0 &&
	world->spylist [spys [work]]->moved_this_turn == 0)
	  return (spys [work]);
...
int spy_lizards_in_hex (World *world, int x, int y, int player)
{
  int work,
      work2;
