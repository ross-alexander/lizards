...
void show_world (int x, int y, int hex_view, int plr, int spy)
{
  int x_mod;
  ...
  int one_hex_disp [6][2] = {{6,13},{8,7},{8,19},{12,7},{12,19},{14,13}};
  ...
  int two_hex_disp [12][2] = {{2,13},{4,7},{4,19},{6,1},{6,25},{10,1},
			      {10,25},{14,1},{14,25},{16,7},{16,19},{18,13}};
  ...
  int one3_hex_disp [6][2] = {{10,19},{12,13},{12,25},{16,13},{16,25},{18,19}};

  int two3_hex_disp [12][2] = {{6,19},{8,13},{8,25},{10,7},{10,31},{14,7},
			       {14,31},{18,7},{18,31},{20,13},{20,25},{22,19}};

  int three_hex_disp [18][2] = {{2,19},{4,13},{4,25},{6,7},{6,31},{8,1},{8,37},
				{12,1},{12,37},{16,1},{16,37},{20,1},{20,37},
				{22,7},{22,31},{24,13},{24,25},{26,19}};
  ...
  if (hex_view == 2)
  {
    ...
    if ((x % 2) == 1)
    {
      ...
      {
	str_replace (14 + x_mod, 10, player[plr].code);
	str_replace (13 + x_mod, 11, "SPYliz");
      }

      show_hex (x, y, 13 + x_mod, 10, plr, 1);
    ...
    if ((x % 2) == 1)
    {
      ...
      if (spy && lizards_in_hex (&hex) == 0)
      {
	str_replace (20 + x_mod, 14, player[plr].code);
	str_replace (19 + x_mod, 15, "SPYliz");
      }

      show_hex (x, y, 19 + x_mod, 14, plr, 1);
...
void show_hex (int x, int y, int scr_x, int scr_y, int plr, int default_all)
{
  ...
  if (hex.hex_owner == plr || default_all == 1)
  {
    if (lizards_in_hex (&hex) > 0 || hex.terrain == DEN || hex.terrain == HOME_DEN)
    {
      str_replace (scr_x + 1, scr_y, player[hex.hex_owner].code);
