...
extern char liz_cp_desc [MAX_COMBAT_POINTS + 1][10];

extern char liz_hunger_desc [MAX_HUNGER_POINTS + 1][10];
  ...
  if (world_hex.hex_owner == order->player)
    ...
    if (world_hex.hex_owner > 0)
      fprintf (fptr, "they see Lizards from '%s' Clan at %s with great clarity.\n",
	       player[world_hex.hex_owner].clan_name, temp_string);
    else
...
int search_chant (int x, int y, order_t *order, int base_liz)
{
  ...
  {
    move_coords (work4, x, y, &x2, &y2);

    get_hex (x2, y2, &world_hex);

    for (work3 = 0; work3 < number_spys; work3 ++)
      if (spy_list [work3]->x_hex == x2 && spy_list [work3]->y_hex == y2 &&
	...
	if (spy_list [work3]->player == order->player)
	{
	  open_player_mesg (order->player, &fptr);
	  ...
	  fprintf (fptr, " - Black Lizards destroy Spy Lizard from '%s' Clan at %s!\n",
		   player[spy_list [work3]->player].clan_name, temp_string);
...
int shunt_chant (int x, int y, order_t *order, int base_liz)
{
  ...
  {
    move_coords (direction, x2, y2, &x3, &y3);
  ...
  if (hex.hex_owner == order->player)
  {
    ...
    fprintf (fptr, "'%s' Clan Lizards at %s are seized by a powerful force and hurled %s!\n",
      player[hex.hex_owner].clan_name, work_string, dir [direction]);
  ...
  {
    in_sight (dest_x, dest_y, players);
	...
	if (already_seen [work] == 1)
	  ...
	  fprintf (fptr, "\n - '%s' Clan Lizards are dumped at %s by a powerful force!\n",
	    player[hex.hex_owner].clan_name, work_string2);
...
int coerce_chant (int x, int y, order_t *order, int base_liz)
{
  ...
  if (order->black_lizards >= lizards_in_hex (&hex))
  {
    ...
    fprintf (fptr, " - %d '%s' Clan Lizards at %s are compelled to join your Clan!\n",
	     lizards_in_hex (&hex), player[hex.hex_owner].clan_name, work_string);
...
int transmute_chant (int x, int y, order_t *order, int base_liz)
{
  ...
  for (work = 0; work < number_orders; work ++)
    if (order_list [work] != order &&
      order_list [work]->x_hex == x &&
      order_list [work]->y_hex == y &&
      order_list [work]->player == hex.hex_owner &&
	...
	order_list [work]->order_type == MOVE)
      {
        ...
        order_list [work]->red_lizards = hex.red_lizards;
        order_list [work]->green_lizards = hex.green_lizards;
        order_list [work]->grey_lizards = hex.grey_lizards;
        order_list [work]->black_lizards = hex.black_lizards;
        order_list [work]->yellow_lizards = hex.yellow_lizards;
      }
...
int freeze_chant (int x, int y, order_t *order, int base_liz)
{
  ...
  if (hex.hex_owner != order->player)
  {
    fprintf (fptr, "'%s' Clan Lizards at %s are FROZEN for this turn!\n",
	     player[hex.hex_owner].clan_name, work_string);
    ...
    fprintf (fptr, "\n - Your Lizards at %s have been FROZEN by '%s' Clan Lizards at %s.\n",
	     work_string, player[order->player].clan_name, work_string2);
...
int summon_chant (int x, int y, order_t *order, int base_liz)
{
  ...
  for (work = 0; work < number_spys; work ++)
	...
	spy_list [work]->player > 0)
    {
      ...
      players [spy_list [work]->player] = 0;
...
int gate_chant (int x, int y, order_t *order, int base_liz)
{
  ...
  if (new_hex.hex_owner != order->player)
  {
	...
	work_string2, lizards_in_hex (&hex), player[order->player].clan_name);
	...
	fprintf (fptr, " - %d Clan '%s' Lizards already occupy the Temple! Combat ensues...\n",
		 lizards_in_hex (&new_hex), player[new_hex.hex_owner].clan_name);
	...
	result = combat (new_hex.red_lizards, new_hex.green_lizards,
			 new_hex.grey_lizards, new_hex.black_lizards,
	...
	if (lizards_in_hex (&hex) == 0)
	{
	  ...
	  fprintf (fptr, " - Your Lizards in Temple at %s killed all of the Clan '%s' Lizards here.\n",
		 work_string2, player[order->player].clan_name);
	  ...
	  capture_lizards (&new_hex, num_captured, &enemy_captured_red,
	  ...
	  fprintf (fptr, " - Your Lizards in Temple at %s killed all of the Clan '%s' Lizards here.\n",
		 work_string2, player[new_hex.hex_owner].clan_name);
	  ...
	  capture_lizards (&hex, num_captured, &captured_red, &captured_green,
