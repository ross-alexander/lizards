...
int main (int argc, char *argv [])
{
  ...
  strcpy (player[0].code, "C-G");

  get_switch_list ();

  get_spy_list ();

  sprintf (out_filename, "%s%s%03d.%s", data_path, POSTSCRIPT_F, world.turn, game_code);
  ...
  for (work = 0; work < players_in_game + 1; work ++)
	...
	switch (player_output_mode [work])
	  ...
	  case 'B':
	    ...
	    sprintf (filename, "%sDPL%02d%03d.%s", disk_data_path, work, world.turn,
		     game_code);
	    ...
	    fprintf (disk_out_fptr, " - %s you control Clan '%s' (%s).\n\n",
		     player_name [work], player[work].clan_name, player[work].code);
  ...
  if ((range = player_hex_see_list [high_end - 1].centred_y -
      ...
      if (hex_per_page > 16)
	{
      ...
      fprintf (fptr, "(%s Clan World View for Lizards! Game %s Turn %d Page 1 of 2. World size 1A to %s) Map_Page_Num\n",
	       player[player].clan_name, game_code, world.turn,
...
void centre_points (anchor_t *base, int centre_x, int centre_y)
{
  ...
  for ( loop = 0; loop < base->num_elements; loop ++ )
	...
	x += 1;
      ...
      if (x < 0) x += world.x;
      if (x >= world.x) x = x % world_x;

      y = ptr->y - centre_y + (world.y / 2);

      if (y < 0) y += world.y;
      if (y >= world.y) y = y % world_y;
