...
int *ProcessFile (FILE*, World*, int*);
...
int* ProcessFile(FILE *in_fptr, World *world, int *no_move_returned_flags)
{
  ...
  while (fgets (work_string, 140, in_fptr) != '\0')
  {
    ...
    strtoken (tmp_string, work_string, " ");

    flay_spaces (work_string);

    for (line_type = 0; line_type < NUM_POSSIBLE_LINES; line_type ++)
      if (strcmp (tmp_string, possible_lines [line_type]) == 0)
	break;
	...
	break;		/* Comment line types, so ignore them! */
      ...
      case 3:
	work1 = atoi (tmp_string);

	if (work1 >= 1 && work1 <= world->num_players)
	{
	  present_player = work1;

	  printf ("Present Player: %d - %s.\n\n", work1,
		  world->player[work1].code);
	  ...
	  for (work3 = 1; work3 < world->num_players + 1; work3 ++)
	  {
		...
		exit (EXIT_FAILURE);
	      ...
	      fprintf (fptr, " - Message from Player %d, '%s' Clan:\n",
		       present_player,
		       world->player[present_player].clan_name);
	...
	work_order.moves [0] = string_to_direction (tmp_string);
...
int main (int argc, char *argv [])
{
  ...
  fprintf(stderr, "Turn %d of game %s\n", world->turn, game_code);
  ...
  sprintf (filename, "%s%s.%s%03d", data_path, game_code, TEXT_ORDS_F, world->turn);

  number_of_orders = 0;
  no_move_returned_flags = (int*)malloc(sizeof(int) * (MAX_PLAYERS + 1));

  for (loop = 1; loop < world->num_players + 1; loop ++)
  {
  ...
  {
    ProcessFile(in_fptr, world, no_move_returned_flags);
    fclose (in_fptr);
  }
  ...
  for (work1 = 0; work1 < number_of_orders; work1 ++)
  {
    put_rec (bin_fptr, sizeof (order_t), work1, order_list [work1]);

    free (order_list [work1]);
  }

  fclose (bin_fptr);

  for (loop = 1; loop < world->num_players; loop ++)
    if (no_move_returned_flags [loop] && world->turn > 0)
  ...
  sprintf (filename, "%s%s%03d.%s", data_path, TEXT_ORDS_F, world->turn,
    game_code);
...
void add_order (order_t *order)
{
  ...
  {
    case SPLIT:
      for (work1 = 0, split_total = 0; work1 < number_of_orders; work1 ++)
	if (order_list [work1]->order_type == SPLIT &&
	    order->player == order_list [work1]->player &&
	    order->x_hex == order_list [work1]->x_hex &&
	    order->y_hex == order_list [work1]->y_hex)
	{
	  split_total += order_list [work1]->red_lizards +
			 order_list [work1]->green_lizards +
			 order_list [work1]->grey_lizards +
			 order_list [work1]->black_lizards +
			 order_list [work1]->yellow_lizards;
	}
    ...
    case MOVE:
      for (work1 = 0; work1 < number_of_orders; work1 ++)
	if (order_list [work1]->order_type == MOVE &&
	    order->player == order_list [work1]->player &&
	    order->x_hex == order_list [work1]->x_hex &&
	    order->y_hex == order_list [work1]->y_hex &&
	    order->red_lizards == order_list [work1]->red_lizards &&
	    order->green_lizards == order_list [work1]->green_lizards &&
	    order->grey_lizards == order_list [work1]->grey_lizards &&
	    order->black_lizards == order_list [work1]->black_lizards &&
	    order->yellow_lizards == order_list [work1]->yellow_lizards)
	{
	  /* Redundant order ignored! */
      ...
      break;
    case SNEAK:
      for (work1 = 0, sneak_total = 0; work1 < number_of_orders; work1 ++)
	if (order_list [work1]->order_type == SNEAK &&
	    order->player == order_list [work1]->player &&
	    order->x_hex == order_list [work1]->x_hex &&
	    order->y_hex == order_list [work1]->y_hex)
	{
    ...
    case CHANT:
      for (work1 = 0; work1 < number_of_orders; work1 ++)
	if (order_list [work1]->order_type == CHANT &&
	    order->player == order_list [work1]->player &&
	    order->x_hex == order_list [work1]->x_hex &&
	    order->y_hex == order_list [work1]->y_hex)
      {
    ...
    case LOB:
      for (work1 = 0; work1 < number_of_orders; work1 ++)
	if (order_list [work1]->order_type == LOB &&
	    order->player == order_list [work1]->player &&
	    order->x_hex == order_list [work1]->x_hex &&
	    order->y_hex == order_list [work1]->y_hex)
      {
    ...
    case BUILD_FERTILE:
      for (work1 = 0; work1 < number_of_orders; work1 ++)
	if (order_list [work1]->order_type == BUILD_FERTILE &&
	    order->player == order_list [work1]->player &&
	    order->x_hex == order_list [work1]->x_hex &&
	    order->y_hex == order_list [work1]->y_hex)
      {
    ...
    case BUILD_DEN:
      for (work1 = 0; work1 < number_of_orders; work1 ++)
	if (order_list [work1]->order_type == BUILD_DEN &&
	    order->player == order_list [work1]->player &&
	    order->x_hex == order_list [work1]->x_hex &&
	    order->y_hex == order_list [work1]->y_hex)
      {
    ...
    case BUILD_BRIDGE:
      for (work1 = 0; work1 < number_of_orders; work1 ++)
	if (order_list [work1]->order_type == BUILD_BRIDGE &&
	    order->player == order_list [work1]->player &&
	    order->x_hex == order_list [work1]->x_hex &&
	    order->y_hex == order_list [work1]->y_hex)
      {
    ...
    case DESTROY_FERTILE:
      for (work1 = 0; work1 < number_of_orders; work1 ++)
	if (order_list [work1]->order_type == DESTROY_FERTILE &&
	    order->player == order_list [work1]->player &&
	    order->x_hex == order_list [work1]->x_hex &&
	    order->y_hex == order_list [work1]->y_hex)
      {
    ...
    case DESTROY_BRIDGE:
      for (work1 = 0; work1 < number_of_orders; work1 ++)
	if (order_list [work1]->order_type == DESTROY_BRIDGE &&
	    order->player == order_list [work1]->player &&
	    order->x_hex == order_list [work1]->x_hex &&
	    order->y_hex == order_list [work1]->y_hex)
      {
