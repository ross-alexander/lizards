...
void add_spy (int *spy_now_shown)
{
  ...
  do
  {
    ...
    select_hex (&x, &y);
  } while (x < 0 || x > world.x || y < 0 || y  > world.y);
  ...
  cprintf ("%03d-P%02d/%3s/%4s/%d old",
	   work, work_spy.player, player[work_spy.player].code,
...
void delete_spy (int *spy_now_shown)
{
  ...
  if (work < 0 || work >= number_spys)
  {
    ...
    cprintf ("%03d-P%02d/%3s/%4s/%d old",
	     work, spy_list [work]->player, player[spy_list [work].code->player],
  ...
  do
  {
    ...
    *y = atoi (work_string);
    ...
    if (strlen (temp_string) > 1)
    {
      ...
      *x = temp_string [0] - 64;
  } while (*x <= 0 || *y <= 0 || *x > world.x || *y > world.y);
