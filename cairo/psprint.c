...
void ps_show_hex (int x, int y, int player, FILE *fptr)
{
  ...
  if (hex.has(BRIDGE) || hex.has(RAFT) || (hex.has(HALF_DEN))
    {
      if (hex.has(BRIDGE))
	fprintf (fptr, BRIDGE_PC);
      else if (hex.has(RAFT))
	fprintf (fptr, RAFT_PC);
      else if (hex.has(HALF_DEN))
  ...
  if (hex.terrain == DEN || hex.terrain == HOME_DEN)
  ...
  else
    {
      ...
      /* --------------------
	 not den and has a band of lizards
	 -------------------- */

      if (lizards_in_hex (&hex) > 0)
	{
	  if ((hex.terrain != SCRUB && hex.terrain != RUIN) || hex.hex_owner == player)
	    {
	      fprintf (fptr, "Small (%s) Owner (%d) Troop_Number\n",
      ...
      else
	{
	  ...
	  if (spy_at (x, y, player))
	    ...
	    {
	      /* --------------------
		 See how owns bridges or temples
		 -------------------- */
	      
	      if ((hex.has(BRIDGE) || hex.terrain == TEMPLE) && hex.hex_owner > 0)
		{
		  fprintf (fptr, "Small (%s) Owner\n", player_code [hex.hex_owner]);
		}
	      else
